...
def select_pts(im,Pts,above_zero):
  ...
  while((k<Pts) & (len(above_zero[0])>0)):
    voxel_idx=np.random.randint(0,len(above_zero[0])-1,1,dtype=np.int64)
...
def deform(im):
  ...
  imgg=sitk.ReadImage('Brats18_2013_0_1_t2.nii.gz')
...
img=nib.load("Brats18_2013_0_1_t2.nii.gz")
im=img.get_data()
...
for i in range(10):
  ...
  deformed=deform(im)
  file_name="elastic"+str(i+1)+".nii.gz"
